...
static const uint8_t floor_num[4][8] =
{
	{
	0b00011000,
	0b00111110,
...
void set_dotmatrix_buffer(uint8_t curr_eleva_state, uint8_t target_floor)
{
	...
	switch (curr_eleva_state) {
		case ELEVA_STOP:
			for(int i=0; i <8; i++){
				display_data[i] = floor_num[get_curr_floor()][0];
			}
			for(int i=1; i < 8; i++){
				for(int j=0; j <8;j++){
					// scroll_buffer[0] = blank
					scroll_buffer[i][j] = floor_num[target_floor][j];
				}
			}
			break;
		case ELEVA_START_BOTTOM_UP: //↑
			for(int i=0; i <8; i++){
				display_data[i] = floor_num[i][0];
			}
			for(int i=1; i < 8; i++){
				for(int j=0; j <8;j++){
					// scroll_buffer[0] = blank
					if(i == 1){
						scroll_buffer[i][j] = up_arrow[i];
						continue;
					}
					scroll_buffer[i][j] = floor_num[target_floor][j];
				}
		...
		case ELEVA_START_TOP_DOWN: // ↓
			...
			for(int i=1; i < 8; i++){
				for(int j=0; j <8;j++){
					// scroll_buffer[0] = blank
					if(i == 1){
						scroll_buffer[i][j] = down_arrow[i];
						continue;
					}
					scroll_buffer[i][j] = floor_num[target_floor][j];
				}
